...
| [`results/nostream-tests.txt`](./results/nostream-tests.txt) | The full [cameri/nostream](https://github.com/cameri/nostream) unit suite passes on my clone — proving the "set up a local Nostream dev environment" half of the prompt. | `cd ../../nostream && npx mocha 'test/**/*.spec.ts' > competency-test/results/nostream-tests.txt 2>&1` |
